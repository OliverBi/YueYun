...
(function($, undefined) {
	...
	jPlayerPlaylist = function(cssSelector, playlist, options) {
		...
		$(this.cssSelector.jPlayer).bind($.jPlayer.event.play, function() {
			...
			$(self).trigger("updateTrackInfo",
					[self.playlist[self.current].artist,self.playlist[self.current].poster]);
		});
	...
	jPlayerPlaylist.prototype = {
		...
		_createListItem: function(media) {
			...
			listItem += "<a href='javascript:;' class='" + this.options.playlistOptions.itemClass + "' tabindex='1'>" + "<span class='jp-track'>" + media.title + "</span>" + (media.id ? " <span class='jp-id' style=display:none;'>" + media.id + "</span>" : "") + (media.artist ? " <span class='jp-artist'>" + media.artist + "</span>" : "") + (media.album ? " <span class='jp-album'>" + media.album + "</span>" : "") + "</a>";
			listItem += "</div></li>";
		...
		add: function(media, playNow) {
			...
			//this.original.push(media);
			//this.playlist.push(media); // Both array elements share the same object pointer. Comforms with _initPlaylist(p) system.
			this.original.splice(0,0,media);
			this.playlist.splice(0,0,media);
